...
import { CalendarClock, Copy, Share2, Users, X } from 'lucide-react';
...
export default function HostProgramPage({ slug, currentUser }: { slug: string; currentUser: User | null }) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-7">
      ...
      {managedGroup && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4" role="presentation"><section role="dialog" aria-modal="true" aria-labelledby="manage-group-title" className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="font-sans text-xs font-extrabold uppercase tracking-wider text-secondary">Manage Group</p><h2 id="manage-group-title" className="mt-1 font-display text-3xl font-bold text-primary">{managedGroup.name}</h2></div><button type="button" onClick={() => setManagedGroup(null)} aria-label="Close Manage Group" className="rounded-full border border-surface-container-high p-2 text-primary"><X className="h-5 w-5" /></button></div><dl className="mt-5 grid gap-3 rounded-2xl bg-surface-container-low p-4 font-sans text-sm font-bold text-on-surface-variant sm:grid-cols-2"><div><dt className="text-[10px] font-extrabold uppercase text-outline">Status</dt><dd>{statusLabel(managedGroup.status)}</dd></div><div><dt className="text-[10px] font-extrabold uppercase text-outline">Orders close</dt><dd>{new Date(managedGroup.closesAt).toLocaleString()}</dd></div><div><dt className="text-[10px] font-extrabold uppercase text-outline">Pickup</dt><dd>{formatPickupDateLabel(managedGroup.pickupDate, data.store.country)} · {managedGroup.pickupSession}</dd></div><div><dt className="text-[10px] font-extrabold uppercase text-outline">Location</dt><dd>{managedGroup.pickupLocationName}</dd></div></dl>{managedGroup.status === 'open' && <div className="mt-4 grid gap-2 sm:grid-cols-2"><button type="button" disabled={busy} onClick={() => updateStatus('closed')} className="rounded-full border border-primary px-5 py-3 font-sans text-xs font-extrabold text-primary disabled:opacity-50">Close Group</button><button type="button" disabled={busy} onClick={() => updateStatus('cancelled')} className="rounded-full bg-error px-5 py-3 font-sans text-xs font-extrabold text-white disabled:opacity-50">Cancel Group</button></div>}<div className="mt-4">{managedGroup.lifetimeOrderCount === 0 ? <button type="button" disabled={busy} onClick={() => cleanupGroup('delete')} className="rounded-full border border-error px-5 py-3 font-sans text-xs font-extrabold text-error disabled:opacity-50">Delete Group</button> : managedGroup.status !== 'open' ? <button type="button" disabled={busy} onClick={() => cleanupGroup('archive')} className="rounded-full border border-primary px-5 py-3 font-sans text-xs font-extrabold text-primary disabled:opacity-50">Archive Group</button> : null}</div><h3 className="mt-7 font-display text-2xl font-bold text-primary">Group orders</h3><p className="mt-1 font-sans text-xs font-bold text-outline">View only. Store payment, refund and fulfilment controls remain with Store operators.</p>{managedOrders.length === 0 ? <p className="mt-4 rounded-2xl bg-surface-container-low p-5 font-sans text-sm font-bold text-on-surface-variant">No customer orders yet.</p> : <div className="mt-4 space-y-3">{managedOrders.map(order => <article key={order.id} className="rounded-2xl border border-surface-container-high p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="font-sans text-sm font-extrabold text-primary">{order.orderNumber}</h4><p className="mt-1 font-sans text-xs font-bold text-on-surface-variant">{order.customerName} · {order.itemCount} item{order.itemCount === 1 ? '' : 's'}</p></div><p className="font-sans text-sm font-extrabold text-primary">{formatRegionCurrency(order.total, order.currency)}</p></div><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-full bg-surface-container px-3 py-1 font-sans text-[10px] font-extrabold uppercase text-primary">Payment: {order.paymentStatus.replaceAll('_', ' ')}</span><span className="rounded-full bg-surface-container px-3 py-1 font-sans text-[10px] font-extrabold uppercase text-primary">Fulfilment: {order.fulfilmentStatus || 'New'}</span></div></article>)}</div>}</section></div>}
    </div>
  );
}
