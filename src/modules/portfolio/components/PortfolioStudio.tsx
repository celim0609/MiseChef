...
import type { Portfolio, PortfolioAbout, PortfolioBasicProfile, PortfolioCertificate, PortfolioContact, PortfolioExperience, PortfolioFeaturedRecipe, PortfolioGalleryItem, PortfolioHero, PortfolioPublishStatus, PortfolioResume, PortfolioSkill } from '../types';
...
export default function PortfolioStudio({ portfolio, recipes, userId, onDraftPortfolioChange, onSavePortfolio }: PortfolioStudioProps) {
  ...
  const updatePublishStatus = (status: PortfolioPublishStatus) => {
    markDraftChanged();
    setDraftPortfolio(current => ({
      ...current,
      visibility: {
        ...(current.visibility || { status: 'private' }),
        status
      }
    }));
  };
    ...
    <div className="space-y-6 pb-6">
      <div className="sticky top-0 z-10 -mx-2 rounded-2xl border border-surface-container-high bg-background/95 p-4 shadow-sm backdrop-blur sm:mx-0 sm:flex sm:items-start sm:justify-between sm:gap-4">
        <div>
          ...
          <h2 className="font-display text-3xl sm:text-4xl font-bold text-primary tracking-tight mt-1">Portfolio Studio</h2>
      ...
      <section className="bg-surface-container-low border border-surface-container-high rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
        <label className="block max-w-sm space-y-2">
          <span className="font-sans text-xs font-extrabold text-primary">Portfolio Status</span>
          <select
            value={draftPortfolio.visibility?.status || 'private'}
            onChange={event => updatePublishStatus(event.target.value as PortfolioPublishStatus)}
        ...
        <div>
          ...
          <p className="mt-2 font-sans text-sm font-bold text-on-surface-variant">Core portfolio details that appear in your hero and overview cards.</p>
        </div>
