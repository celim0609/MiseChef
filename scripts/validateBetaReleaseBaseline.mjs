...
const requiredSourceMarkers = [
  ...
  ['functions/index.js', 'export const processResumeImportJob = onDocumentCreated({', 'asynchronous Resume Import worker'],
  ['functions/index.js', "document: 'resumeImportJobs/{jobId}'", 'Resume Import server job trigger'],
  ['src/services/gemini.ts', 'startResumeToPortfolioJob', 'Resume Import callable job creation'],
  ['src/services/resumeImportJobs.ts', 'subscribeToResumeImportJob', 'Resume Import client job subscription'],
  ['src/services/resumeImportJobsModel.ts', "export type ResumeImportJobStatus = 'pending' | 'processing' | 'done' | 'failed'", 'Resume Import job data model'],
  ['firestore.rules', 'match /resumeImportJobs/{jobId}', 'Resume Import owner-scoped status rules'],
  ['tests/resumeImportJobAccessControl.test.mjs', 'clients cannot create or modify server-owned resume import jobs', 'Resume Import Firestore Rules regression'],
