...
if (!candidateRoot || !existsSync(path.join(candidateRoot, '.git'))) {
  throw new Error('Beta run 33583791849 follow-up requires --candidate-root pointing to the exact candidate checkout.');
}
...
const protectedGateSha = process.env.MISECHEF_BETA_RUN_33583791849_RECOVERY_GATE_SHA || '';
if (!/^[0-9a-f]{40}$/.test(protectedGateSha) || git(controllerRoot, ['rev-parse', 'HEAD']) !== protectedGateSha) {
  throw new Error('The follow-up recovery controller does not match the protected Beta environment gate SHA.');
}
resolveBetaRun33583791849RecoveryMode({
  confirmation: process.env.MISECHEF_BETA_RUN_33583791849_RECOVERY_CONFIRMATION,
  authorization: process.env.MISECHEF_BETA_RUN_33583791849_RECOVERY_AUTHORIZATION,
...
if (head !== BETA_RUN_33583791849.candidateCommit || sourceTree !== BETA_RUN_33583791849.candidateSourceTree) {
  throw new Error('The follow-up recovery candidate is not the exact authorized SHA and source tree.');
}
...
if (firebaseRc.projects?.beta !== BETA_PROJECT_ID) {
  throw new Error('Follow-up recovery is not using the pinned Beta Firebase project.');
}
...
assertPinnedFirebaseTargetedRedeployBehavior({
  version: firebaseVersion,
  prepareSource: readFileSync(path.join(firebaseToolsRoot, 'lib', 'deploy', 'functions', 'prepare.js'), 'utf8'),
  plannerSource: readFileSync(
    path.join(firebaseToolsRoot, 'lib', 'deploy', 'functions', 'release', 'planner.js'),
    'utf8'
  )
});
...
const readPostRunState = async live => assertBetaRun33583791849PostRunState({
  head,
  sourceTree,
  ...
  assetProof: await readLiveAssetProof({
    origin: betaOrigin,
    asset: BETA_RUN_33583791849.manifest.entryAsset
  })
});
...
const stateBefore = await readPostRunState(liveBefore);
...
const stateBeforeDeploy = await readPostRunState(liveBeforeDeploy);
if (stateBefore.fingerprint !== stateBeforeDeploy.fingerprint) {
  throw new Error('The audited post-run state changed while validation was running. Follow-up recovery refused.');
}

const nonce = randomBytes(32).toString('hex');
const sessionPath = path.join(os.tmpdir(), `misechef-beta-run-33583791849-recovery-${process.pid}-${nonce}.json`);
...
try {
  ...
  lockCreated = true;
  writeFileSync(lockDescriptor, JSON.stringify({ pid: process.pid, head, recovery: BETA_RUN_33583791849.id }));
  ...
  const beforeFunctions = readBetaFunctionState();
  const beforeServices = await readCloudRunServiceState({ firebaseToolsRoot });
  const targetedResult = spawnSync('firebase', targetedRepairFirebaseArgs(), {
    cwd: candidateRoot,
  ...
  if (targetedResult.status !== 0) {
    throw new Error(
      `Protected targeted Function repair failed once with exit code ${targetedResult.status}; recovery stopped and no retry was attempted.`
    );
  }
  ...
  if (fullResult.status !== 0) {
    throw new Error(
      `Canonical complete Beta resource deployment failed once with exit code ${fullResult.status}; no retry was attempted.`
    );
  }
  ...
  console.log(`Protected Beta follow-up recovery converged all resources to ${head}.`);
} finally {
