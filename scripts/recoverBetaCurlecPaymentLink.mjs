...
assertAuthority({
  ...
  documentedBaseline: JSON.parse(readFileSync(path.join(candidateRoot, 'config/beta-release-baseline.json'), 'utf8')).minimumCommit,
  head,
  isAncestor: (ancestor, descendant) => spawnSync('git', ['merge-base', '--is-ancestor', ancestor, descendant], { cwd: candidateRoot }).status === 0
});
...
const lockPath = path.resolve(candidateRoot, git(['rev-parse', '--git-common-dir']), 'misechef-beta-deployment.lock');
let locked = false;
try {
  ...
  const deployed = spawnSync('firebase', ['deploy', '--project', 'beta', '--only', FULL_BETA_RESOURCE_PLAN.join(',')], { cwd: candidateRoot, env: { ...process.env, FIREBASE_DEPLOY_TARGET: 'beta', MISECHEF_BETA_PROTECTED_BASELINE: MANDATORY_BETA_BASELINE, MISECHEF_BETA_DEPLOY_SESSION_FILE: session, MISECHEF_BETA_DEPLOY_SESSION_NONCE: nonce }, stdio: 'inherit' });
