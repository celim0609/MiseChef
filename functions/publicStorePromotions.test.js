...
test('public projection exposes only active winning normal product display data', () => {
  ...
  assert.deepEqual(result.promotions, [{ productId: 'a', name: 'Offer', type: 'percentage', savings: 3, terms: { percentageOff: 30, minimumOrderAmount: 0 }, originalPrice: 10, estimatedPrice: 7 }]);
...
test('Buy 1 Get 1 appears using its first qualifying quantity', () => {
  ...
  assert.deepEqual(result.promotions, [{
    productId: 'a', name: 'Offer', type: 'buy_x_get_y', savings: 6,
